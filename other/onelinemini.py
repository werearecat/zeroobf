...
def obfuscate(content):
    encrypted_content = encryptcode(content)
    compiled_code = compile(encrypted_content, "<string>", "exec")
...
def main():
    parser = argparse.ArgumentParser(description='1-line Obfuscator')
    ...
    for i in range(args.layer):
        code = obfuscate(code)
        print(f"Layer {i + 1}/{args.layer} obfuscated\n{len(code)} bytes!")
