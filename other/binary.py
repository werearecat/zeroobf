def obfcode(input_file, output_file):
    ...
    binary_code_hidden = binary_code.replace("0", "\u005F").replace("1", "_").replace("1", "\u200C").replace(" ", "\u200D")

    # Tạo mã nguồn để giải mã và thực thi
    obfuscated_code = f"""
# https://github.com/werearecat/zeroobf
# made with chatgpt :)
exec("".join(chr(int(b, 2)) for b in "{binary_code_hidden}".replace("\u005F", "0").replace("_", "1").replace("\u200D", " ").split()))
    """
